...
const validateInitConfig = async (config) => {
    ...
    if (!config[dbConfigKeys.dbAuthUser].length) {
        throw new Error(`Invalid config: Required field - '${dbConfigKeys.dbAuthUser}'`);
    }
    validatedConfig.dbAuthUser = config[dbConfigKeys.dbAuthUser];
    
    if (!config[dbConfigKeys.dbAuthPwd].length) {
        throw new Error(`Invalid config: Required field - '${dbConfigKeys.dbAuthPwd}'`);
    }
    validatedConfig.dbAuthPwd = config[dbConfigKeys.dbAuthPwd];
    
    if (!config[dbConfigKeys.dbHost].length) {
        throw new Error(`Invalid config: Required field - '${dbConfigKeys.dbHost}'`);
    }
    ...
    if (!config[dbConfigKeys.dbName].length) {
        throw new Error(`Invalid config: Required field - '${dbConfigKeys.dbName}'`);
    }
    validatedConfig.dbName = config[dbConfigKeys.dbName];

    if (config[dbConfigKeys.dbbackupPath].length) {
        ...
        throw new Error(`Invalid config: Required field - '${dbConfigKeys.dbbackupPath}'`);
    }
    validatedConfig.dbbackupPath = config[dbConfigKeys.dbbackupPath];

    if (!config[dbConfigKeys.dbBackupTime].length) {
        throw new Error(`Invalid config: Required field - '${dbConfigKeys.dbBackupTime}'`);
    }
    if(config[dbConfigKeys.dbBackupTime].indexOf(":") == -1) {
        throw new Error(`Invalid config: Invalid ${dbConfigKeys.dbBackupTime} - '${config[dbConfigKeys.dbBackupTime]}'`);
    }
    ...
    if (backupTimeHours.length == 0 || backupTimeMinutes.length == 0 || backupTimeHours.length > 2 || backupTimeMinutes.length > 2) {
        throw new Error(`Invalid config: Invalid ${dbConfigKeys.dbBackupTime} - '${config[dbConfigKeys.dbBackupTime]}'`);
    }
    ...
    if (isNaN(config[dbConfigKeys.dbNoOfDays]) || Number(config[dbConfigKeys.dbNoOfDays]) == 0) {
        throw new Error(`Invalid config: Not a valid Number - ${config[dbConfigKeys.dbNoOfDays]} for the field '${dbConfigKeys.dbNoOfDays}'`);
    }
    ...
    if (isNaN(config[dbConfigKeys.dbNoOfWeeks]) || Number(config[dbConfigKeys.dbNoOfWeeks]) == 0) {
        throw new Error(`Invalid config: Not a valid Number - ${config[dbConfigKeys.dbNoOfWeeks]} for the field '${dbConfigKeys.dbNoOfWeeks}'`);
    }
    ...
    if (isNaN(config[dbConfigKeys.dbNoOfMonths]) || Number(config[dbConfigKeys.dbNoOfMonths]) == 0) {
        throw new Error(`Invalid config: Not a valid Number - ${config[dbConfigKeys.dbNoOfMonths]} for the field '${dbConfigKeys.dbNoOfMonths}'`);
    }
